...
// API URL - 로컬 테스트용
// const API_BASE_URL = 'http://10.0.2.2:8000/api'  // 에뮬레이터용 로컬
const API_BASE_URL = 'http://3.27.28.175/api'  // AWS 서버 (주석 해제 시 AWS 사용)
...
export const authAPI = {
  ...
  publicSignup: (data: any) => api.post('/public/signup/', data), // 승인대기 회원신청
}
...
export const communityAPI = {
  // 게시글
  getPosts: (params?: any) => api.get('/posts/', { params }),  // gym_only, gym_id, category, search
  ...
  // 좋아요
  likePost: (id: number, memberId: number) => api.post(`/posts/${id}/like/`, { member: memberId }),
  unlikePost: (id: number, memberId: number) => api.post(`/posts/${id}/unlike/`, { member: memberId }),
